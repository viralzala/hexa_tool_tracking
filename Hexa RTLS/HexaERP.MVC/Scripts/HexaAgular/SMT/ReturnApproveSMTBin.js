...
//
app.controller("ReturnApproveSMTBinCtrl", function ($timeout, $scope, $http, $window) {


    $scope.empIds = [];
    $scope.SubmitApprovalRequest = function () {
        ...
        $http({
            method: 'POST',
            url: '../ReturnApproveSMTBin/ReturnSMTBinRequest',
    ...
    function initializeComponets() {
        ...
        $http({
            method: 'GET',
            url: '../ReturnApproveSMTBin/GetReturnApproveSMTBin'
        }).then(function successCallback(response) {
            console.log(response.data);
            if (response.data.Flag == true) {
