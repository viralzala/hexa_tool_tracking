...
//
app.controller("SearchByGatewayCtrl", function ($timeout, $scope, $http, $window) {
    //initializeComponets();
    ...
    function _() {
        if ($(`#PartNumber`).val()) {
            $http({
                method: 'GET',
                url: '../SearchByGateway/GetSMTProdcutInfo',
    ...
    function initializeComponets() {
        setInterval(function () {
            $http({
                method: 'GET',
                url: '../SearchByGateway/GatewayConnect'
            }).then(function successCallback(response) {
                if (response.data.Flag == true) {
                    $scope.SmtProducts = response.data._product;
                    toastr.success(`${response.data.message}`);
