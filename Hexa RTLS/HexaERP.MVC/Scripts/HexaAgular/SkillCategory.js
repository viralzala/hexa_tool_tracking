...
//
app.controller("SkillCategoryCtrl", function ($timeout, $scope, $http, $window) {
    initializeComponets();
    ...
    $scope.SaveFormCollData = function () {
        if (angular.isUndefined($scope.SkillCategory) || $scope.SkillCategory === null) {
            toastr.error('Enter Skill Category');
            return false;
        }
    ...
    $('body').on('click', '#EditIdata', function () {
        ...
        //console.log(isp.mSiteMasterId);
        $http({
            method: 'GET',
            url: '../SkillCategory/Edit',
            params: { id: isp.mSkillCategoryId }
        }).then(function successCallback(response) {
            console.log(response.data);
            if (response.data.Flag == true) {
                $scope.SkillCategory = response.data.Idata.SkillCategory;
                $scope.mSkillCategoryId = response.data.Idata.mSkillCategoryId;
    ...
    $('body').on('click', '#Deletebtn', function () {
        ...
        if (answer) {
            ...
            //for row data
            var isp = table.row(row).data();
            DeleteRecord(isp.mSkillCategoryId);
        }
    ...
    $scope.EditFormCollData = function () {
        if (angular.isUndefined($scope.SkillCategory) || $scope.SkillCategory === null || angular.isUndefined($scope.mSkillCategoryId) || $scope.mSkillCategoryId === null) {
