...
//
app.controller("SiteMasterCtrl", function ($timeout, $scope, $http, $window) {
    initializeComponets();
    ...
    $scope.SaveFormCollData = function () {
        if (angular.isUndefined($scope.Site) || $scope.Site === null) {
    ...
    $('body').on('click', '#EditIdata', function () {
        ...
        $http({
            method: 'GET',
            url: '../SiteMaster/Edit',
            params: { id: isp.mSiteMasterId }
        }).then(function successCallback(response) {
            //console.log(response.data);
            if (response.data.Flag == true) {
                $scope.Site = response.data.Idata.Site;
                $scope.mSiteMasterId = response.data.Idata.mSiteMasterId;
    ...
    $('body').on('click', '#Deletebtn', function () {
        ...
        if (answer) {
            ...
            //for row data
            var isp = table.row(row).data();
            DeleteRecord(isp.mSiteMasterId);
        }
