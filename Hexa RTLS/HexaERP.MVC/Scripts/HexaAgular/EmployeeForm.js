        ...
        content_height: function (t, i) {
            ...
            t.length && (t.steps({
                headerTag: "h3", bodyTag: "section", transitionEffect: "slideLeft", trigger: "change", onInit: function (i, e) {
                    ...
                    var tEmployeeTagId = $("#tEmployeeTagId").val();

                    if (tEmployeeTagId === 'undefined' || tEmployeeTagId === null || tEmployeeTagId == "") {
                        $.ajax({
                            type: 'POST',
                            url: '../EmployeeTag/Create',
                            data: iData,
                    ...
                    else if (tEmployeeTagId !== 'undefined' && tEmployeeTagId != null && tEmployeeTagId != "") {
                        var iData = JSON.stringify(i.serializeObject(), null, 2);
                        console.log(iData);
                        $.ajax({
                            type: 'POST',
...
function putPorts() {
    UIkit.modal.confirm('Are you sure to Connect Desktop Reader?', function () {
        ...
        console.log(_Portm);
        $.get("../EmployeeTag/PutStart", { _Port: _Portm }, function (response) {
            //console.log(response);
...
function DataTap() {
    modal = UIkit.modal.blockUI('<div class=\'uk-text-center\'>Wait moment...<br/><img class=\'uk-margin-top\' src=\'../Content/assets/img/spinners/spinner.gif\' alt=\'\'>');
    $.getJSON("../EmployeeTag/getDataTap", function (data) {
        $("#RFID").val(); $("#PORTID").val();
        if (data.Flag == false) {
            setTimeout(function () {
                modal.hide()
...
function BindJqueryTable() {
    $.get("../EmployeeTag/getData", function (data) {
