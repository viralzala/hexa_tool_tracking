...
//
function GetReaders() {
    $.getJSON("../AssetTag/getGetReadersData", function (data) {
...
//
function GetUnits() {
    $.getJSON("../AssetTag/getGetUnitsData", function (data) {
...
//
function GetIteams() {
    $.getJSON("../AssetTag/getIteamsData", function (data) {
        $('#mIteamMasterId').kendoComboBox({
...
//
function GetGroups() {
    $.getJSON("../AssetTag/getGroupsData", function (data) {
        $('#mGroupMasterId').kendoComboBox({
...
//
function GetTypes() {
    $.getJSON("../AssetTag/getTypesData", function (data) {
        //console.log(data);
        $('#mIteamTypeMasterId').kendoComboBox({
...
//
function GetFloors() {
    $.getJSON("../AssetTag/getGetFloorsData", function (data) {
...
function GetRooms() {
    ...
    if (_fid != "") {
        $.getJSON("../AssetTag/getGetRoomsData", { FloorId: $('#mFloorMasterId').val() }, function (data) {
            if (data == "") {
                $('#mRoomMasterId').kendoComboBox({
...
$(function () {
    ...
    $("#btnsave").click(function () {
        var tAssetTagIds = $("#tAssetTagIds").val();
        var UIDs = $("#UIDs").val();
        var RFIDs = $("#RFIDs").val();
        var QTY = $("#Stockup").val();
        UIkit.modal.confirm('Are you sure to update quantity?', function () {
            ...
            if (tAssetTagIds == "" || tAssetTagIds == null || UIDs == "" || UIDs == null || RFIDs == "" || RFIDs == null || QTY == "" || QTY == null) {
                setTimeout(function () {
                    modal.hide()
                ...
                $.get("../AssetTag/setQTYData", { tAssetTagId: tAssetTagIds, UID: UIDs, RFID: RFIDs, qty: $("#Stockup").val() }, function (data) {
                    setTimeout(function () {
                        modal.hide()
...
function GetData() {
    ...
    table.clear().draw();
    $.getJSON("../AssetTag/getData", function (data) {
                ...
                { "mData": "Stock" },
                {
                    "render": function (aaData, type, row, meta) {
                        ...
                        if (row.bStock != null) {
                            ...
                            return '<a href="#mailbox_new_message" onclick="setqty(\'' + row.tAssetTagId + '\',\'' + row.UID + '\',\'' + row.RFID + '\');"  data-uk-tooltip="{cls:"uk-tooltip-small",pos:"left"}" data-uk-modal="{center:true}"><h3>' + parseFloat(row.bStock) + '</h3></a>';
                        }
                        else {
                            return '<span class="uk-badge uk-badge-warning">Not Assined</span>';
                        }
                ...
                { "mData": "RoomName" },
                {
                    "render": function (aaData, type, row, meta) {
                        ...
                        if (row.FloorName != null) {
                            ...
                            return '<span class="uk-badge uk-badge-warning">Floor:<b>' + row.FloorName + '</b></span><br/><span class="uk-badge uk-badge-warning">Room Name:<b>' + row.RoomName + '</b></span>';
                        }
                        else {
                            return '<span class="uk-badge uk-badge-warning">Not Assined</span>';
                        }
...
function setqty(tAssetTagId, UID, RFID) {
...
$(document).on('click', '#Deletebtn', function (e) {
    ...
    if (answer) {
        $.get("../AssetTag/DeleteData", { ID: Ids }, function (data) {
            GetData();
        });
    }
    else {
